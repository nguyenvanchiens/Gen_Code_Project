...
const GUIDES = [
  ...
  {
    id: 'claude-cmd-guide',
    title: 'Tổng hợp lệnh Claude CLI trong phát triển dự án',
    icon: '🤖',
    description: 'Từ triển khai ý tưởng → tính năng → review → test → commit → merge request',
    steps: [
      {
        title: 'Bước 1 — Chuẩn bị môi trường & khởi tạo dự án',
        content: [
          'Mở terminal tại thư mục gốc của dự án, sau đó khởi động Claude Code:',
          '<code>claude</code> &mdash; khởi động Claude Code tương tác trong thư mục hiện tại',
          '<code>claude --model claude-opus-4-7</code> &mdash; chọn model cụ thể (Opus mạnh hơn, Haiku nhanh hơn)',
          '<code>claude -p "mô tả task"</code> &mdash; chạy một lần không tương tác (pipe mode)',
          '',
          'Sau khi vào Claude Code, khởi tạo tài liệu dự án:',
          '<code>/init</code> &mdash; Claude đọc toàn bộ codebase và tạo file <code>CLAUDE.md</code> với tài liệu, conventions, cấu trúc dự án',
          '<code>/config</code> &mdash; cấu hình model mặc định, theme giao diện, các cài đặt chung',
          '',
          '<em>Tip: <code>CLAUDE.md</code> là "bộ nhớ dài hạn" của Claude về dự án &mdash; commit file này vào git để cả team dùng chung.</em>',
        ],
      },
      {
        title: 'Bước 2 — Triển khai ý tưởng & lên kế hoạch',
        content: [
          'Mô tả tính năng bằng ngôn ngữ tự nhiên để Claude phân tích trước khi code:',
          '<strong>Yêu cầu Claude lập kế hoạch:</strong> <em>"Tôi muốn thêm tính năng X, hãy phân tích codebase và lên kế hoạch implement"</em>',
          '',
          'Kích hoạt Plan Mode để Claude không code ngay mà chỉ lập kế hoạch:',
          '<code>/plan</code> &mdash; bật Plan Mode, Claude sẽ nghiên cứu và trình bày kế hoạch để bạn duyệt trước khi thực thi',
          '',
          'Các câu hỏi hữu ích trong giai đoạn này:',
          '&bull; <em>"Các file nào liên quan đến tính năng X?"</em>',
          '&bull; <em>"Cách implement tốt nhất là gì, có những trade-off gì?"</em>',
          '&bull; <em>"Có những rủi ro hoặc breaking change nào cần chú ý?"</em>',
          '&bull; <em>"Viết acceptance criteria cho tính năng này"</em>',
        ],
      },
      {
        title: 'Bước 3 — Phát triển tính năng với gitlab-flow',
        content: [
          'Sử dụng skill <code>gitlab-flow</code> để tự động hóa toàn bộ quy trình từ Jira task đến Merge Request:',
          '<code>/gitlab-flow</code> &mdash; kích hoạt quy trình chuẩn: đọc task Jira &rarr; tạo branch &rarr; implement &rarr; commit &rarr; push &rarr; tạo MR',
          '',
          'Cung cấp task ID để Claude lấy context từ Jira:',
          '<em>"Implement task WRA-40"</em> &mdash; Claude tự đọc Jira, tạo branch <code>feature/WRA-40-ten-tinh-nang</code>, code theo yêu cầu',
          '',
          'Tạo branch thủ công nếu cần:',
          '<code>git checkout -b feature/WRA-40-ten-tinh-nang</code> &mdash; convention: <code>feature/TASK-ID-mo-ta-ngan</code>',
          '<code>git checkout -b fix/WRA-41-sua-loi-x</code> &mdash; branch sửa lỗi',
          '',
          '<em>Tip: Yêu cầu Claude "implement từng bước nhỏ" thay vì "implement toàn bộ" để dễ review và rollback.</em>',
        ],
      },
      {
        title: 'Bước 4 — Review code',
        content: [
          'Claude Code có nhiều cấp độ review tùy mức độ quan trọng của thay đổi:',
          '',
          '<strong>Review toàn bộ branch (local):</strong>',
          '<code>review-branch</code> &mdash; review tất cả những thay đổi trên branch hiện tại so với main',
          '',
          '<strong>Review theo phạm vi:</strong>',
          '<em>"review các thay đổi chưa commit"</em> &mdash; review working tree hiện tại (chưa <code>git add</code>)',
          '<em>"review code trong commit abc1234"</em> &mdash; review theo commit hash cụ thể',
          '<em>"review commit mới nhất"</em> &mdash; review <code>HEAD</code>',
          '',
          '<strong>Review theo nhánh:</strong>',
          '<em>"review tất cả thay đổi trên nhánh <code>feature/hncw-311-fix-log-survey</code>"</em>',
          '<em>"review diff của nhánh này so với main"</em>',
          '',
          '<strong>Review theo file / class:</strong>',
          '<em>"review file <code>Src/Apps/.../SomeFile.cs</code>"</em>',
          '<em>"review toàn bộ class <code>SurveyService</code>"</em>',
          '',
          '<strong>Review theo tính năng / logic:</strong>',
          '<em>"review logic xử lý survey log (HNCW-311)"</em>',
          '<em>"review phần xác thực token refresh"</em>',
          '',
          '<strong>Review bảo mật:</strong>',
          '<code>/security-review</code> &mdash; kiểm tra chuyên sâu: SQL injection, XSS, OWASP Top 10, lộ secret, logic lỗi bảo mật',
          '',
          '<strong>Multi-agent cloud review (tốn credit):</strong>',
          '<code>/ultrareview</code> &mdash; nhiều AI agent độc lập review song song, tổng hợp kết quả &mdash; dùng cho PR quan trọng',
          '<code>/ultrareview 123</code> &mdash; review PR/MR theo số (cần có GitHub/GitLab remote)',
          '',
          '<em>Thứ tự nên dùng: <code>review-branch</code> hàng ngày &rarr; <code>/security-review</code> trước merge &rarr; <code>/ultrareview</code> cho feature lớn.</em>',
        ],
      },
      {
        title: 'Bước 5 — Test',
        content: [
          'Yêu cầu Claude viết và chạy test:',
          '<em>"Viết unit test cho function <code>calculateTotal</code> trong file <code>order.service.ts</code>"</em>',
          '<em>"Chạy toàn bộ test suite và giải thích các lỗi đang fail"</em>',
          '<em>"Thêm integration test cho API endpoint POST /api/orders"</em>',
          '',
          'Claude tự chạy lệnh test phù hợp theo stack:',
          '<code>npm test</code> / <code>npm run test:watch</code> &mdash; Node.js / React',
          '<code>dotnet test</code> &mdash; .NET / ASP.NET Core',
          '<code>pytest</code> / <code>python -m pytest -v</code> &mdash; Python',
          '<code>go test ./...</code> &mdash; Go',
          '',
          'Tự động hóa test lặp lại:',
          '<code>/loop 3m "chạy test và báo cáo kết quả"</code> &mdash; Claude tự chạy test mỗi 3 phút, thông báo khi có lỗi',
          '',
          '<em>Tip: Yêu cầu Claude "chạy test trước và sau khi sửa" để xác nhận fix không làm regression.</em>',
        ],
      },
      {
        title: 'Bước 6 — Commit theo Conventional Commits',
        content: [
          '<strong>Dùng skill <code>/commit</code> để tạo commit chuẩn:</strong>',
          '<code>/commit JIRA-ID</code> &mdash; tạo commit đầy đủ có body (title + mô tả chi tiết thay đổi)',
          '<code>/commit JIRA-ID --quick</code> &mdash; tạo commit không có body (chỉ title, nhanh hơn)',
          '<em>Ví dụ: <code>/commit HNCW-311</code> &rarr; Claude phân tích diff, tạo commit message gắn task ID</em>',
          '',
          'Claude tự phân tích diff và tạo commit với message chuẩn:',
          '<code>git add &lt;files&gt;</code> &mdash; Claude chọn đúng file liên quan, tránh commit file nhạy cảm',
          '<code>git commit -m "feat(WRA-40): thêm tính năng thanh toán online"</code>',
          '',
          '<strong>Conventional Commits format:</strong>',
          '<code>feat(scope): mô tả</code> &mdash; tính năng mới',
          '<code>fix(scope): mô tả</code> &mdash; sửa bug',
          '<code>refactor(scope): mô tả</code> &mdash; tái cấu trúc, không thêm tính năng',
          '<code>test(scope): mô tả</code> &mdash; thêm/sửa test',
          '<code>docs(scope): mô tả</code> &mdash; cập nhật tài liệu',
          '<code>chore(scope): mô tả</code> &mdash; cấu hình, dependencies',
          '',
          '<em>Tip: Mỗi commit nên là một thay đổi logic độc lập &mdash; dễ revert và dễ review.</em>',
        ],
      },
      {
        title: 'Bước 7 — Tạo Merge Request trên GitLab',
        content: [
          '<strong>1. Cài skill <code>gitlab-flow</code> nội bộ</strong> &mdash; liên hệ team để lấy file skill và cài vào Claude Code',
          '<strong>2. Cài <code>glab</code> CLI trên máy</strong> &mdash; xem hướng dẫn chi tiết tại tab <strong>🔧 Cài đặt glab CLI</strong>',
          '<strong>3. Tạo MR:</strong> <em>"create a merge request"</em> &rarr; Claude dùng <code>glab mr create</code> tự sinh title/description chuẩn và tạo MR',
        ],
      },
    ],
    summary: [
      '<code>claude</code> &mdash; khởi động Claude Code CLI',
      '<code>/init</code> &mdash; tạo CLAUDE.md tài liệu hóa dự án',
      '<code>/plan</code> &mdash; lập kế hoạch trước khi implement',
      '<code>/gitlab-flow</code> &mdash; quy trình tự động từ Jira task &rarr; MR',
      '<code>review-branch</code> &mdash; review tất cả thay đổi trên branch hiện tại',
      '<code>/commit JIRA-ID</code> &mdash; tạo commit đầy đủ có body gắn task ID',
      '<code>/commit JIRA-ID --quick</code> &mdash; tạo commit nhanh không có body',
      '<code>/security-review</code> &mdash; kiểm tra bảo mật OWASP',
      '<code>/ultrareview</code> &mdash; multi-agent review chuyên sâu',
      '<code>/loop Xm "task"</code> &mdash; lặp tác vụ tự động theo interval',
      '<em>"create a merge request"</em> &mdash; tạo Merge Request GitLab qua Claude (cần skill <code>gitlab-flow</code> + <code>glab</code> CLI)',
    ],
  },
];
